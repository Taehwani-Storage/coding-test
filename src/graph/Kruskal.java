...
public class Kruskal {
    ...
    public void solution() throws IOException {
        ...
        for (int i = 0; i < edgeCount; i++) {
            StringTokenizer edegTokenize = new StringTokenizer(reader.readLine());
            edges[i][0] = Integer.parseInt(edegTokenize.nextToken());
            edges[i][1] = Integer.parseInt(edegTokenize.nextToken());
            edges[i][2] = Integer.parseInt(edegTokenize.nextToken());
        }
        // 준비 단계

        // 1. 간선들을 가중치 기준에서 정렬
        Arrays.sort(edges, Comparator.comparingInt(edge -> edge[2]));
        // 2. 간선들을 가중치 순서대로 순회하면서 고름
        int totalWeight = 0;
        int picked = 0; // N-1개만 고르더라도 끝냄
        ...
        for (int i = 0 ; i < edgeCount; i++) {
            ...
            if (findSet(start) != findSet(end)) {
                ...
                union(start, end);
                picked++; // 간선 골랐음 표시
                totalWeight += edges[i][2]; // 총 가중치 업데이트
